...
void push(struct head *stack_p, int data) {     // create an element and push it to the top of the stack
    struct element *new_element=calloc(1,sizeof(struct element)),*p;   // p points to the current top
    if (stack_p->size == 0){                        // imply that no element in the stack
        stack_p->next = new_element;
    }
    else{                                           // if there are any element in the stack
        p=stack_p->top;                             // let old top's next point to new top
        p->next=new_element;
    }
    stack_p->top = new_element;
    new_element->data = data;
    stack_p->size++;
}

struct element* pop(struct head *stack_p) {     // return the element which is popped from the stack
    struct element *the_pop=stack_p->top , *pre=stack_p->next;
    if (stack_p->size == 0) return NULL;
    else if (stack_p->size == 1){
        stack_p->top = NULL;
        stack_p->next = NULL;
    }
    else {
        for (;pre->next != the_pop ; pre=pre->next);        // let pre be the second of the stack
        pre->next=NULL;
        stack_p->top = pre;
    }
    stack_p->size--;
